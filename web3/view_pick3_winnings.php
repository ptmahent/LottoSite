...
$htmlFormStartOut .= '<form name="frmViewLotto" id="frmViewLotto" method="get" action="view_pick3_winnings.php">'
...
$htmlMainCont .= '<table id="lottery_result" >'
			  . '<thead><tr><th>Draw Date</th>
				...
				<th >Straight Match - Count</th><th> Prize</th><th >Box Match - Count</th><th>Prize</th></tr></thead><tbody>';
/*
print "st DT: " . $st_date;
print "\n ed DT: " . $ed_date;
print "\n stRowNum: " . $st_row_num;
 ...
 * 
 */
$db_res = $OLGLottery->OLGOnPick3WinningsGetDraw($st_date, $ed_date, $st_row_num, $ed_row_num);
//print_r($db_res);
if (is_array($db_res)) {
...
$htmlMainCont .=  '<tr ><td id="drawDate" nowrap>' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>"
				...
				. '<td id="winCount">' . $db_row["m_3_s_count"] . '</td>'
				. '<td id="win_amt">' . money_format('%(#12n',$db_row["m_3_s_prze_amt"]) . '</td>'
				. '<td id="winCount">' . $db_row["m_3_b_count"] . '</td>'
				. '<td id="win_amt">' . money_format('%(#12n',$db_row["m_3_b_prze_amt"]) . '</td>'
				. '</tr>';
