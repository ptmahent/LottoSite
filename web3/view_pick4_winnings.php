...
$htmlFormStartOut .= '<form name="frmViewLotto" id="frmViewLotto" method="get" action="view_pick4_winnings.php">'
...
$htmlMainCont .= '<table id="lottery_result" >'
			  . '<thead><tr><th>Draw Date</th>
				...
				<th>N3</th>
				<th>N4</th>
				<th >Straight - Count</th><th>Prize</th><th >Box - Count</th><th>Prize</th>'
			  . '<th >4  Way Box</th><th>Prize</th><th>6  Way Box</th><th>Prize</th><th>12 Way Box</th><th>Prize</th>'
			  . '<th>24 Way Box</th><th>Prize</th></tr></thead><tbody>';
/*
print "st DT: " . $st_date;
print "\n ed DT: " . $ed_date;
print "\n stRowNum: " . $st_row_num;
 ...
 * 
 */
$db_res = $OLGLottery->OLGOnPick4WinningsGetDraw($st_date, $ed_date, $st_row_num, $ed_row_num);
//print_r($db_res);
if (is_array($db_res)) {
  foreach ($db_res as $db_row) {
	$htmlMainCont .=   '<tr ><td id="drawDate" nowrap>' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>"
				 ...
				 . '<td id="drawNumber">' .  $db_row["snum4"] . '</td>'
			 	  . '<td id="winCount">' . $db_row["m_4_s_count"] . '</td>'
				  . '<td id="win_amt" nowrap>' . money_format('%(#12n',$db_row["m_4_s_prze_amt"]) . '</td>'
				  . '<td id="winCount">' . $db_row["m_4_b_count"] . '</td>'
				  . '<td id="win_amt" nowrap>' . money_format('%(#12n',$db_row["m_4_b_prze_amt"]) . '</td>'
				  . '<td id="winCount">' . $db_row["m_4_4w_box_count"] . '</td>'
				  . '<td id="win_amt" nowrap>' . money_format('%(#12n',$db_row["m_4_4w_box_prze_amt"]) . '</td>'
				  . '<td id="winCount">' . $db_row["m_4_6w_box_count"] . '</td>'
				  . '<td id="win_amt">' . money_format('%(#12n',$db_row["m_4_6w_box_prze_amt"]) . '</td>'
				  . '<td id="winCount">' . $db_row["m_4_12w_box_count"] . '</td>'
				  . '<td id="win_amt">' . money_format('%(#12n',$db_row["m_4_12w_box_prze_amt"]) . '</td>'
				  . '<td id="winCount">' . $db_row["m_4_24w_box_count"] . '</td>'
				  . '<td id="win_amt">' . money_format('%(#12n',$db_row["m_4_24w_box_prze_amt"]) . '</td>'
				  . '</tr>';
  }
}
