  ...
  function alc_fetch_single_draw($drawdate = "") {
    ...
    //    $sOct2011_BonusDraw_Hdr = "<img src=\"../images/en/649BonusLogoENG.png\" \/>";
     //   $sOct2011_BonusDraw_Hdr = "<img src=\"..\/images\/en\/649BonusLogoENG\.png\" \/>";
        ...
        */
        $sOct2011_BonusDraw = "(6 of 6)" . $srgE_td . $srgB_td . $srgB_td . "(\d{1,2})-(\d{1,2})-(\d{1,2})-(\d{1,2})-(\d{1,2})-(\d{1,2})" . $srgE_td . $srgB_td . $srgB_td . "\s*(\d.*)\s" . $srgE_td . $srgB_td . $srgB_td . "([^<]*)" . $srgE_td;
        $swinloc = "(\d*)\s*-\s*(.*)";
        ...
        foreach ($html_tr_list as $html_tr) {
          ...
          if ($b649_Oct11_Prze_m == 1 &&
            ...
            $isequencenum = 1;
          	...
          	$na649BonusDrawId = $naLottery->na649GetDrawId($drawdate, $isequencenum);
          	if (!$na649BonusDrawId) {
          		$na649BonusDrawId = $naLottery->na649Add($drawdate, $idrawnum, $snum1, $snum2, $snum3, $snum4, $snum5, $snum6, $snumbonus, "", "", "", "", $isequencenum);
          	}
