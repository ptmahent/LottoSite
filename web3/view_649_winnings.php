...
$htmlFormStartOut .= '<form name="frmViewLotto"  id="frmViewLotto" method="get" action="view_649_winnings.php">'
...
$htmlMainCont .= '<table id="lottery_result" >'
			  . '<thead><tr><th>Draw Date</th>
				...
				<th>N5</th>
				<th>N6</th>
				<th>BONUS</th><th nowrap colspan="2">6/6 </th><th nowrap > Winning Locations</th><th colspan="2">5/6 + </th><th nowrap> Winning Locations</th><th colspan="2">5/6 </th><th colspan="2">4/6 </th>'
			  . '<th colspan="2">3/6 </th><th colspan="2">2/6 + </th>'
			  . '</tr></thead><tbody>';
/*
print "st DT: " . $st_date;
print "\n ed DT: " . $ed_date;
...
if (is_array($db_res)) {
  foreach ($db_res as $db_row) {
	...
	if ($drawDateINT >= $oct2011START && $drawDateINT <= $oct2011END) {
		...
		$htmlMainCont .= '<tr ><td id="drawDate" nowrap>Bonus Draw<br />' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>"
			  . '<td id="drawNumber">' . $db_row["snum1"] . '</td>'
			  . '<td id="drawNumber">' . $db_row["snum2"] . '</td>'
			  . '<td id="drawNumber">' . $db_row["snum3"] . '</td>'
			  . '<td id="drawNumber">' . $db_row["snum4"] . '</td>'
		...
		if ($db_row["isequencenum"] == 0) {
			$htmlMainCont .= '<td id="winCount">' . $db_row["m_6_count"] . '</td>'
						  . '<td id="win_amt" nowrap>' . money_format('%(#12n',$db_row['m_6_prze_amt']) . '</td>'
						  . '<td nowrap id="win_amt" nowrap>';
			...
			if (is_array($db_loc_res)) {
				foreach ($db_loc_res as $db_6_loc) {
					if ($db_6_loc["wnum_m"] == 6) {
						$htmlMainCont .=  $db_6_loc["win_loc"] . " - " . $db_6_loc["wcount"];
						$htmlMainCont .=  " - " . money_format('%(#12n',$db_6_loc["win_prze_amt"]) . "<br />";
					}
		...
		$htmlMainCont .= '<td id="winCount">' . $db_row["m_5_b_count"] . '</td>'
					  . '<td id="win_amt" nowrap>' . money_format('%(#12n',$db_row['m_5_b_prze_amt']) . '</td>'
					  . '<td nowrap id="win_amt" nowrap>';
		if (is_array($db_loc_res)) {
		  foreach ($db_loc_res as $db_5_b_loc) {
			if ($db_5_b_loc["wnum_m"] == 5) {
			  $htmlMainCont .=  $db_5_b_loc["win_loc"] . " - " . $db_5_b_loc["wcount"];
			  $htmlMainCont .=  " - " . money_format('%(#12n',$db_5_b_loc["win_prze_amt"]) . "<br />";
			}
	  ...
	  //print_r($db_loc_res);
		...
		$htmlMainCont .= '</td><td id="winCount">' . $db_row["m_5_count"] . '</td>'
					  . '<td id="win_amt" nowrap>' . money_format('%(#12n', $db_row["m_5_prze_amt"]) . '</td>';
		
		$htmlMainCont .= '<td id="winCount">' . $db_row["m_4_count"] . '</td><td id="win_amt" nowrap>' .
		money_format('%(#12n', $db_row["m_4_prze_amt"]) . '</td>';
		
		$htmlMainCont .= '<td id="winCount">' . $db_row["m_3_count"] . '</td>'
					  . '<td id="win_amt" nowrap>' . money_format('%(#12n', $db_row["m_3_prze_amt"]) . '</td>';
		
		$htmlMainCont .= '<td id="winCount">' . $db_row["m_2_b_count"] . '</td>'
					  . '<td id="win_amt">' . money_format('%(#12n', $db_row["m_2_b_prze_amt"]) . '</td>';
		} else {
			$htmlMainCont .= '<td id="winCount">' . $db_row["m_6_count"] . '</td>'
						  . '<td id="win_amt" nowrap>' . money_format('%(#12n',$db_row['m_6_prze_amt']) . '</td>'
						  . '<td nowrap id="win_amt" nowrap>';
		...
		// print_r($db_loc_res);
			...
			$htmlMainCont .= '<td id="winCount" colspan="11">&nbsp;</td>';
