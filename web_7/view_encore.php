...
$htmlFormStartOut .= '<form name="frmViewLotto" id="frmViewLotto" method="get" action="view_encore.php">'
 ...
 * 
 */
$db_res = $OLGLottery->OLGEncoreGetDraw($st_date, $ed_date, $st_row_num, $ed_row_num);
//print_r($db_res);
if (is_array($db_res)) {
  ...
  $htmlMainCont .=  '<tr ><td id="drawDate" nowrap>' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>";
	for ($i = 0; $i <= 9; $i++) {
		...
		$td_cont = "";	
		if (array_key_exists("snum1", $db_row) && $db_row["snum1"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .= "<br />";
		}
		if (array_key_exists("snum2", $db_row) && $db_row["snum2"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .= "<br />";
		}
		if (array_key_exists("snum3", $db_row) && $db_row["snum3"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .=  "<br />";
		}
		if (array_key_exists("snum4", $db_row) && $db_row["snum4"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .=  "<br />";
		}
		if (array_key_exists("snum5", $db_row) && $db_row["snum5"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .= "<br />";
		}
		if (array_key_exists("snum6", $db_row) && $db_row["snum6"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .= "<br />";
		}
		if (array_key_exists("snum7", $db_row) && $db_row["snum7"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .=  "<br />";
		}
		if (array_key_exists("snum8", $db_row) && $db_row["snum8"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .=  "<br />";
		}
		if (array_key_exists("snum9", $db_row) && $db_row["snum9"] == $i) {
			$td_cont .= $i . "<br />";
		} else {
			$td_cont .=  "<br />";
		}
		$htmlMainCont .= "<td id='drawNumber'>" . $td_cont . "</td>";
	}
	$htmlMainCont .= "</tr>";
...
$smarty->display('templates/view_numbers.tpl');
