...
$htmlFormStartOut .= '<form name="frmViewLotto" id="frmViewLotto" method="get" action="view_lottario.php">'
 ...
 * 
 */
$db_res = $OLGLottery->OLGLottarioGetDraw($st_date, $ed_date, $st_row_num, $ed_row_num);
//print_r($db_res);
if (is_array($db_res)) {
  foreach ($db_res as $db_row) {
		...
		for ($i = 1; $i <= 49; $i++) {
			...
			if ($db_row["snum1"] == $i) {
				...
				$td_cont = $i;
			} elseif ($db_row["snumbonus"] == $i) {
				$td_cont = $i;
			}
			
			if ($db_row["eb_snum1"] == $i) {
				if ($td_cont == "") {
					$td_cont .= "&nbsp;<hr />" . $i;
				} else {
					$td_cont .= "<hr />" . $i;
				}
			} elseif ($db_row["eb_snum2"] == $i) {
				if ($td_cont == "") {
					$td_cont .= "&nbsp;<hr />" . $i;
				} else {
					$td_cont .= "<hr />" . $i;
				}		
			} elseif ($db_row["eb_snum3"] == $i) {
				if ($td_cont == "") {
					$td_cont .= "&nbsp;<hr />" . $i;
				} else {
					$td_cont .= "<hr />" . $i;
				}		
			} elseif ($db_row["eb_snum4"] == $i) {
				if ($td_cont == "") {
					$td_cont .= "&nbsp;<hr />" . $i;
				} else {
					$td_cont .= "<hr />" . $i;
			...
			if ($td_cont == "") {
				...
				if ($db_row["snumbonus"] != $i) {
					$htmlMainCont .= "<td id='drawNumber'>" . $td_cont . "</td>";
				} else {
					$htmlMainCont .= "<td id='matchBonusNumber'>" . $td_cont . "</td>";
				}
			}
		}
		$htmlMainCont .= '</tr>';
