...
$htmlMainCont .= '<table id="lottery_result" >'
	...
	foreach ($sPokerClass as $pkrClassId => $pkrClass) {
		
		foreach ($sPokerCards as $pkrCardId => $pkrCard) {
		
			$htmlMainCont .= '<th id="head_drawNumber">' . $pkrCard[1] . $pkrClass[1] . '</th>';
		}
...
if (is_array($db_res)) {
 ...
 $htmlMainCont .=  '<tr ><td id="drawDate" nowrap>' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>";
	...
	foreach ($sPokerClass as $pkrClassId => $pkrClass) {
		...
		foreach ($sPokerCards as $pkrCardId => $pkrCard) {
			if (strtolower($db_row["scard1"]) == strtolower($pkrCard[1] . $pkrClass[1])) {
				$htmlMainCont .= '<td id="drawNumber">'  . $pkrCard[1] . $pkrClass[1] . '</td>';			
			} elseif (strtolower($db_row["scard2"]) == strtolower($pkrCard[1] . $pkrClass[1] )) {
				$htmlMainCont .= '<td id="drawNumber">' . $pkrCard[1] . $pkrClass[1] . '</td>';			
			
			}  elseif (strtolower($db_row["scard3"]) == strtolower( $pkrCard[1] . $pkrClass[1] )) {
				$htmlMainCont .= '<td id="drawNumber">' . $pkrCard[1] . $pkrClass[1] . '</td>';			
			
			} elseif (strtolower($db_row["scard4"]) == strtolower( $pkrCard[1] . $pkrClass[1] )) {
				$htmlMainCont .= '<td id="drawNumber">' . $pkrCard[1] . $pkrClass[1] . '</td>';			
			
			} elseif (strtolower($db_row["scard5"]) == strtolower( $pkrCard[1] . $pkrClass[1] )) {
				$htmlMainCont .= '<td id="drawNumber">' . $pkrCard[1] . $pkrClass[1] . '</td>';			
