...
{wlocation: INT}
    ...
    function na649Add($drawdate, $idrawnum, $snum1, $snum2, $snum3, $snum4, $snum5, $snum6, $snumbonus, $sregion = "", $drawNo ="", $sdrawDate = "", $spielID = "", $isequencenum = "") {
    ...
    function na649Modify($olddrawdate, $newdrawdate = "", $na649id = "", $idrawnum = "", $snum1 = "", $snum2 = "", $snum3 = "", $snum4 = "", $snum5 = "", $snum6 = "", $snumbonus = "", $drawNo = "", $sdrawDate = "", $spielID = "", $isequencenum = "") {
        ...
        if ($isequencenum != "") {
        	$ssql .= sprintf(", `isequencenum` = %u", $isequencenum);
        }
    ...
    function na649GetDrawId($drawdate, $isequencenum = "") {
      if (!$this->db_obj) {
          $this->db_obj = new db();
        }
        ...
        if ($isequencenum != "") {
        	$ssql .=  sprintf(" AND isequencenum = %u", $isequencenum);
    ...
    function na649GetSingleDraw($drawdate, $isequencenum= "", $na649id = "") {
        if (!$this->db_obj) {
          $this->db_obj = new db();
        }
          ...
          if (is_array($db_data)) {
            ...
            foreach ($db_data as $db_row) {
              ...
              $smatch_wins[$irow_cnt] = array(
                ...
                "match_bonus" => 0,
                "match_bonus_num" => 0,
                "isequencenum" => $db_row["isequencenum"]
              );
              //print_r($smatch_wins);
    ...
    function na649GetDrawWinnings($st_drawdate, $ed_drawdate, $st_row_num, $ed_row_num) {
        ...
        $ssql = sprintf("
              ...
              SELECT `na649winningid`, na_649.*, na_649.`drawdate`, na_649.`na649id`, `m_6_count`, `m_6_amount`, 
                ...
                649_winning.na649id = na_649.na649id AND na_649.drawdate >= '%s' AND na_649.drawdate <= '%s' ",
                $st_drawdate, $ed_drawdate);
      $ssql .= sprintf(" order by na_649.drawdate, na_649.isequencenum");
      $db_res = $this->db_obj->fetch($ssql);
      //print "SQL: " . $ssql;
      if (is_array($db_res)) {
        return $db_res;
