 ...
 * 
  ...
  if ($on49["drawStartDate"] != null) {
    $sGameStartDate = $on49["drawStartDate"];
  } else {
...
$htmlFormStartOut .= '<form name="frmViewLotto" id="frmViewLotto" method="get" action="view_49.php">'
...
$htmlMainCont .= '<div id="game_draw_body"><table border="0" colspan="0" width="100%">'
				. '<tr><td>Draw Date</td><td colspan="6">Winning Number</td>'
				. '<td>Bonus</td><td>&nbsp;</td></tr>';
/*
print "st DT: " . $st_date;
print "\n ed DT: " . $ed_date;
print "\n stRowNum: " . $st_row_num;
 ...
 * 
 */
$db_res = $OLGLottery->OLG49GetDraw($st_date, $ed_date, $st_row_num, $ed_row_num);
//print_r($db_res);
if (is_array($db_res)) {
  foreach ($db_res as $db_row) {
  
$htmlMainCont .= '<tr class="49Row"><td>' . date('Y-m-d',strtotime($db_row["drawdate"])) . '</td>'
			  . '<td>' . $db_row["snum1"] . '</td>'
			  . '<td>' . $db_row["snum2"] . '</td>'
			  . '<td>' . $db_row["snum3"] . '</td>'
			  . '<td>' . $db_row["snum4"] . '</td>'
...
$htmlMainCont .= '</table>';
$htmlMainCont .= '<table border="0"><tr><td >' .  $strOutput . '</td>'
			  . '<td>&nbsp;</td></tr></table></div>';
