 ...
 * 
  ...
  if ($naMax["drawStartDate"] != null) {
    $sGameStartDate = $naMax["drawStartDate"];
  } else {
...
$htmlFormStartOut .= '<form name="frmViewLotto" id="frmViewLotto" method="get" action="view_max_winnings.php">'
...
. '<tr><td>Draw Date</td><td>iSeq #</td><td>Winning Number</td>'
. '<td colspan="3">7/7 Match</td><td colspan="3">6/7 + Bonus Match</td>'
. '<td colspan="2">6/7 Match</td><td colspan="2">5/7 Match</td>'
. '<td colspan="2">4/7 Match</td><td colspan="2">3/7 + Bonus Match</td>'
. '<td colspan="2">3/7 Match</td></tr>';
 ...
 * 
 */
//$db_res = $naLottery->naMaxWinningsGetDraw($st_date, $ed_date, $st_row_num, $ed_row_num);
$db_res = $naLottery->naMaxWinningsGetDraw($st_date, $ed_date, $st_row_num);
...
$htmlMainCont .= '<tr class="649Row">'
   ...
   . '<td>' . $db_row["isequencenum"] . '</td>'
   . '<td>';

$htmlMainCont .=  $db_row["snum1"] . "-" . $db_row["snum2"] . "-" . $db_row["snum3"] . "-" . $db_row["snum4"] . "-" . $db_row["snum5"] . "-" . $db_row["snum6"] . "-" . $db_row["snum7"];

if ($db_row["isequencenum"] == 0) {
  $htmlMainCont .=  " Bonus: " . $db_row["snumbonus"];
}
$htmlMainCont .= ' </td>'
. '<td>' . $db_row["m_7_count"] . '</td>'
. '<td><span class="win_amt">' . money_format('%(#12n',$db_row['m_7_prze_amt']) . '</span></td>'
. '<td>';

$db_loc_res = $naLottery->naMaxWinningGetLocs($db_row['namaxwinningid']);
if (is_array($db_loc_res)) {
  foreach ($db_loc_res as $db_7_loc) {
    if ($db_7_loc["wnum_m"] == 7) {
      $htmlMainCont .=  $db_7_loc["win_loc"] . " - " . $db_7_loc["wcount"];
      $htmlMainCont .=  " - <span class='win_amt'>" . money_format('%(#12n',$db_7_loc["win_prze_amt"]) . "</span><br />";
    }
...
$htmlMainCont .= '</td>'
. '<td>' . $db_row["m_6_b_count"] . '</td>'
. '<td><span class="win_amt">' . money_format('%(#12n',$db_row['m_6_b_prze_amt']) . '</span></td>'
. '<td>';

if (is_array($db_loc_res)) {
  foreach ($db_loc_res as $db_6_b_loc) {
    if ($db_6_b_loc["wnum_m"] == 6) {
      $htmlMainCont .=  $db_6_b_loc["win_loc"] . " - " . $db_6_b_loc["wcount"];
      $htmlMainCont .=  " - <span class='win_amt'>" . money_format('%(#12n',$db_6_b_loc["win_prze_amt"]) . "</span><br />";
    }
...
$htmlMainCont .= '</td><td>' . $db_row["m_6_count"] . '</td>'
. '<td><span class="win_amt">' . money_format('%(#12n', $db_row["m_6_prze_amt"]) . '</span></td>';

$htmlMainCont .= '<td>' . $db_row["m_5_count"] . '</td>'
. '<td><span class="win_amt">' . money_format('%(#12n', $db_row["m_5_prze_amt"]) . '</span></td>';

$htmlMainCont .= '<td>' . $db_row["m_4_count"] . '</td>'
. '<td><span class="win_amt">' . money_format('%(#12n', $db_row["m_4_prze_amt"]) . '</span></td>';

$htmlMainCont .= '<td>' . $db_row["m_3_b_count"] . '</td><td>'
. '<span class="win_amt">' . money_format('%(#12n', $db_row["m_3_b_prze_amt"]) . '</span></td>';

$htmlMainCont .= '<td>' . $db_row["m_3_count"] . '</td>'
. '<td><span class="win_amt">' . money_format('%(#12n', $db_row["m_3_prze_amt"]) . '</span></td></tr>';
