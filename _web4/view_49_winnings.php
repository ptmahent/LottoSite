...
$htmlMainCont = "";
$htmlMainCont .= '<div id="game_draw_body"><table border="1" colspan="0" cellspan="1" width="100%">'
			  . '<tr><td>Draw Date</td><td>&nbsp;</td><td >6/6 Count</td><td >6/6 Amount</td>'
			  . '<td >5/6 + Count</td><td >5/6 + Amount</td><td >5/6 Count</td>'
			  . '<td >5/6 Amount</td><td >4/6 Count</td><td >4/6 Amount</td>'
			  . '<td >3/6 Count</td><td >3/6 Amount</td></tr>';
/*
print "st DT: " . $st_date;
print "\n ed DT: " . $ed_date;
print "\n stRowNum: " . $st_row_num;
...
if (is_array($db_res)) {
  foreach ($db_res as $db_row) {
	...
	$htmlMainCont .= '<tr class="649Row"><td>' . date('Y-m-d',strtotime($db_row["drawdate"])) . '</td>'
				  . '<td nowrap>' . $db_row["snum1"] . "-" . $db_row["snum2"]  . "-" .  $db_row["snum3"]  . "-" .  $db_row["snum4"] 
				  . "-" . $db_row["snum5"] . "-" . $db_row["snum6"]  . " Bonus: " . $db_row["snumbonus"];

	$htmlMainCont .= '</td>';
	$htmlMainCont .= '<td>' . $db_row["m_6_count"] . '</td>'
				  . '<td><span class="win_amt">' . money_format('%(#12n',$db_row['m_6_prze_amt']) . '</span></td>'
				  . '<td>' . $db_row["m_5_b_count"] . '</td>'
				  . '<td><span class="win_amt">' . money_format('%(#12n',$db_row['m_5_b_prze_amt']) . '</span></td>'
	...
	$htmlMainCont .= '<td>' . $db_row["m_3_count"] . '</td>'
				  . '<td><span class="win_amt">' . money_format('%(#12n', $db_row["m_3_prze_amt"]) . '</span></td>'
				  . '</tr>';
  }
}
