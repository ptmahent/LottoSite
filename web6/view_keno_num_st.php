...
session_start();
  ...
  require_once("../inc/validform/libraries/ValidForm/class.validform.php");
	...
	require_once($incdir . "/incStatGen.php");
...
$OLGCombLottery = new CombOLGLottery();
...
if (is_array($db_res)) {
  ...
  foreach ($db_res as $db_row) {
	 ...
	 $htmlMainCont .=  '<tr ><td id="drawDate" nowrap>' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>";
  	...
  	$db_keno_st_rs = $OLGCombLottery->OLGCombKenoStatsGet(date('Y-m-d', strtotime($db_row['drawdate'])));
  	
  	if (is_array($db_keno_st_rs)) {
  		
  
		 $htmlMainCont .=  '<tr ><td id="drawDate" nowrap>' . date('Y-m-d',strtotime($db_row["drawdate"])) . "</td>";
  	
  		$db_rs_comb_keno = $OLGCombLottery->OLGCombKenoGet($db_keno_st_rs['icomb_keno_id']);
  		
  		for ($i = 1; $i <= 70; $i++) {
			if ($db_rs_comb_keno["snum1"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum1"] . '</td>';
			}elseif ($db_rs_comb_keno["snum2"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum2"] . '</td>';	
		
			} elseif ($db_rs_comb_keno["snum3"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum3"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum4"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum4"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum5"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum5"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum6"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum6"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum7"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum7"] . '</td>';	
		
			} elseif ($db_rs_comb_keno["snum8"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum8"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum9"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum9"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum10"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum10"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum11"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum11"] . '</td>';	
			}  elseif ($db_rs_comb_keno["snum12"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum12"] . '</td>';	
		
			} elseif ($db_rs_comb_keno["snum13"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum13"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum14"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum14"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum15"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum15"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum16"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum16"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum17"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum17"] . '</td>';	
		
			} elseif ($db_rs_comb_keno["snum18"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum18"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum19"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum19"] . '</td>';	
			} elseif ($db_rs_comb_keno["snum20"] == $i) {
				$htmlMainCont .= '<td id="drawNumber">' . $db_rs_comb_keno["snum20"] . '</td>';	
			} else {
				$htmlMainCont .= '<td id="drawNumber">&nbsp;</td>';
			}
