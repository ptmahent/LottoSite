...
$htmlMainCont .= '<table id="lottery_result" >'
			  . '<thead><tr><th>Draw Date</th>
				...
				<th>N5</th>
				<th>N6</th>
				<th>BONUS</th><th >6/6 Count</th>'
			  . '<th >6/6 Amount</th><th> Winning Locations</th><th >5/6 + Count</th>'
			  . '<th >5/6 + Amount</th><th> Winning Locations</th><th >5/6 Count</th>'
			  . '<th >5/6 Amount</th><th >4/6 Count</th><th >4/6 Amount</th>'
			  . '<th >3/6 Count</th><th >3/6 Amount</th><th >2/6 + Count</th><th >2/6 + Amount</th>'
			  . '</tr></thead><tbody>';
/*
print "st DT: " . $st_date;
print "\n ed DT: " . $ed_date;
...
if (is_array($db_res)) {
  foreach ($db_res as $db_row) {
	...
	$htmlMainCont .= '<td id="winCount">' . $db_row["m_6_count"] . '</td>'
			  	  . '<td id="win_amt">' . money_format('%(#12n',$db_row['m_6_prze_amt']) . '</td>'
				  . '<td nowrap id="win_amt">';
  ...
  //print_r($db_loc_res);
	...
	$htmlMainCont .= '</td><td id="winCount">' . $db_row["m_5_b_count"] . '</td>'
				  . '<td id="win_amt">' . money_format('%(#12n',$db_row['m_5_b_prze_amt']) . '</td>'
				  . '<td nowrap id="win_amt">';
	if (is_array($db_loc_res)) {
...
$htmlMainCont .= '</td><td id="winCount">' . $db_row["m_5_count"] . '</td>'
			  . '<td id="win_amt">' . money_format('%(#12n', $db_row["m_5_prze_amt"]) . '</td>';

$htmlMainCont .= '<td id="winCount">' . $db_row["m_4_count"] . '</td><td id="win_amt">' .
money_format('%(#12n', $db_row["m_4_prze_amt"]) . '</td>';

$htmlMainCont .= '<td id="winCount">' . $db_row["m_3_count"] . '</td>'
			  . '<td id="win_amt">' . money_format('%(#12n', $db_row["m_3_prze_amt"]) . '</td>';
